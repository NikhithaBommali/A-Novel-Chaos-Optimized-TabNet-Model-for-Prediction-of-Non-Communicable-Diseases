...
interface UserLoginProps {
  onLogin: (name: string, role: "user") => void
  onSwitchToSignup: () => void
  onBack: () => void
}

export default function UserLogin({ onLogin, onSwitchToSignup, onBack }: UserLoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (email && password) {
      try {
        ...
        const response = await import('@/lib/api').then(m => m.auth.login(formData, "user"));
        ...
        <Card className="border-2 border-purple-200 bg-white/95 backdrop-blur-xl shadow-2xl">
          <CardHeader className="pt-8 pb-6">
            ...
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <CardTitle className="text-3xl text-slate-900">User Login</CardTitle>
